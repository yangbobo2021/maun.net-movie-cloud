...
export default function ReelShortDetailPage() {
  ...
  if (isLoading) return <div className="min-h-screen pt-24 text-center">Memuat ReelShort...</div>;

  let book: any = data && (data.bookId ? data : data.data?.book);

  if (error || !book) {
    const waLink = "https://wa.me/6281245511900?text=Halo%20Rhezza%20Maun,%20saya%20mau%20beli%20voucher%20ReelShort";
    return (
      <div className="min-h-screen pt-24 px-4 flex items-center justify-center">
        <div className="max-w-md w-full glass p-8 rounded-3xl text-center space-y-6 shadow-2xl border border-primary/20">
          ...
          <h1 className="text-2xl font-bold font-display">ReelShort Premium</h1>
          <p className="text-muted-foreground text-sm leading-relaxed">Gunakan voucher aktif untuk menonton. Hubungi Rhezza Maun (081245511900).</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
          ...
          <div className="space-y-6">
            ...
            <Link href={`/watch/reelshort/${id}`} className="inline-flex items-center gap-2 px-8 py-3 rounded-full font-semibold text-primary-foreground shadow-lg hover:scale-105 transition-all" style={{ background: "var(--gradient-primary)" }}>
